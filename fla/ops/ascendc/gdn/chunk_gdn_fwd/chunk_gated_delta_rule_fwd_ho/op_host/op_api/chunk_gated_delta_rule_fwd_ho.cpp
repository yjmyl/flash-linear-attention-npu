 ...
 */
#include "chunk_gated_delta_rule_fwd_ho.h"
...
OP_TYPE_REGISTER(ChunkGatedDeltaRuleFwdHO);
...
const std::array<const aclTensor *, 2> ChunkGatedDeltaRuleFwdHO(
    const aclTensor *q,
    const aclTensor *k,
    const aclTensor *w,
    const aclTensor *u,
...
{
    L0_DFX(ChunkGatedDeltaRuleFwdHO, q, k, w, u, g, gkOptional, initialStateOptional,
    ...
    if ((cuSeqlensOptional != nullptr && cuSeqlens == nullptr) ||
        ...
        OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "Convert FwdHO metadata failed.");
        return {nullptr, nullptr};
    }

    const aclError ret = ADD_TO_LAUNCHER_LIST_AICORE(
        ChunkGatedDeltaRuleFwdHO,
        OP_INPUT(q, k, w, u, g, gkOptional, initialStateOptional, cuSeqlens, chunkIndices),
        OP_OUTPUT(oOut, finalStateOut),
        OP_ATTR(outputFinalState, chunkSize, scale));
    if (ret != ACLNN_SUCCESS) {
        OP_LOGE(ACLNN_ERR_PARAM_INVALID, "ADD_TO_LAUNCHER_LIST_AICORE ChunkGatedDeltaRuleFwdHO failed.");
        return {nullptr, nullptr};
    }
